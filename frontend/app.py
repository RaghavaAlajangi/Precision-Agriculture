...
crop_img_files = [
    os.path.join(CROP_IMG_PATH, f) for f in os.listdir(CROP_IMG_PATH)
]
...
def build_fig(df):
    ...
    fig = go.Figure(
        go.Parcats(
            dimensions=[
                {"label": "Nitrogen", "values": list(df["Nitrogen"])},
                {"label": "Phosphorous", "values": list(df["Phosphorous"])},
                {"label": "Potassium", "values": list(df["Potassium"])},
                {"label": "Temperature", "values": list(df["Temp(°C)"])},
                {"label": "Humidity", "values": list(df["Humidity(%)"])},
                {"label": "PH", "values": list(df["PH"])},
                {"label": "Rainfall", "values": list(df["Rainfall(mm)"])},
                {"label": "Crop_name", "values": list(df["Crop_name"])},
            ],
                ...
                html.Div(
                    ...
                    children=[
                        ...
                        html.Div(
                            [
                                html.H2(
                                    "PH value of the soil (between 2-9):",
                                    style={"font-size": "15px"},
                                ),
                                dcc.Input(
                                    id="ph",
                    ...
                    children=[
                        ...
                        html.Div(
                            [
                                ...
                                html.Div(
                                    id="crop_name", className="six columns"
                                ),
                            ],
                            className="row",
                        ),
...
def store_inputs(N, P, K, temp, hum, ph, rain):
    ...
    if (
        len(features_str) == 7
        and None not in features_str
        and "" not in features_str
    ):
        return {
            "N": N,
            "P": P,
...
@app.callback(
    [Output("prediction_image", "src"), Output("crop_name", "children")],
    ...
    if stored_inputs is not None:
        ...
        if "submit_button" in trigger:
            return [
                ...
                html.P(
                    [
                        f"Recommended crop:  {pred_crop_name.capitalize()}",
                        html.Br(),
